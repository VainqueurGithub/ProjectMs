...
class Cart
{
     ...
     */
    public function add($id, $name = null, $price = null, $quantity = null, $sejour = null, $prixtotal = null, $adherant = null, $prestation = null, $user = null, $Paterner = null, $attributes = array(), $conditions = array(), $associatedModel = null)
    {
    ...
    public function sejourExist($Type){
    ...
    public function TotalCart($Content){
         $TotalCart=0;
        foreach ($Content as $Item) {
            $TotalCart += $Item->prixtotal;
        }
        return $TotalCart;
    }

    public function TotalCartSejour($Content){
         $TotalCartSejour=0;
        foreach ($Content as $Item) {
            if ($Item->sejour==1) {
                 $TotalCartSejour += $Item->prixtotal;
            }
        }
        return $TotalCartSejour;
    }

    public function QuantityCartSejour($Content){
         $QuantityCartSejour=0;
        foreach ($Content as $Item) {
            if ($Item->sejour==1) {
                 $QuantityCartSejour += $Item->quantity;
            }
        }
        return $QuantityCartSejour;
    }

    public function NbreItemHorsSejour($Content){
         $NbreItemHorsSejour=0;
        foreach ($Content as $Item) {
            if ($Item->sejour==0) {
                 $NbreItemHorsSejour += 1;
            }
        }
        return $NbreItemHorsSejour;
    }

    public function ClearCart($Content){
        foreach ($Content as $Item) {
            if ($Item->sejour==0) {
                Cart::clear($Item->id);
            }
